...
**默认：开启**，`mode: promote`（Hermes 式直接写入）。`finclaw init` 会写入 stub `config.yaml`，并可交互询问；若需更保守，用 `finclaw learning disable` 或改为 `stage` / `observe`。
...
**没有** `finclaw chat --learning` 开关。偏好写在 profile `config.yaml`（或 `AI_INFRA_RS_LEARNING_*`），然后照常 `finclaw chat`。

## profile `config.yaml`

`finclaw init` 会种子写入（也可用 `finclaw config path` 编辑）：
...
| `mode` | 适用场景 |
| --- | --- |
| **`promote`** | **默认。** 立即写入 live 记忆与 agent 技能（Hermes 式）。 |
| **`stage`** | 候选写入 **pending**；用 `finclaw learning` **批准或拒绝**。 |
| **`observe`** | 审阅运行但**不写盘**（仅记录意图）。最安全干跑。 |
...
- 交互：确认是否启用（默认是）并选择 mode（默认 promote）。
- 非交互：stub 保持 enabled+promote；可用 `--no-learning` 或 `--learning-mode stage|observe|promote`。
- `--skip-learning-prompt`：跳过交互提问，保留 stub 默认。
...
| `enable` / `disable` / `set-mode` | 写入 profile `config.yaml`。 |
...
```bash
finclaw config path
# init 已种子 learning.enabled: true、mode: promote；可选 memory_nudge_turns: 2
