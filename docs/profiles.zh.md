...
查看并刷新运行时消费的 canonical 身份信封（`identity.aieos.yaml`）：
...
在 `workspaces/…/agents/<id>/workspace/` 下编辑 Markdown，而无需手写深路径：
...
`identity` 层的解析顺序与 `identity show` 一致（先试工作区再找 profile 兜底），必要时可脚手架 `IDENTITY.md`。详见 `finclaw agent --help`。
...
finclaw profile diff --check   # CI 友好的简要漂移摘要
```
